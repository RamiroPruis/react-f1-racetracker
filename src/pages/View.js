...
function View() {
    ...
    useEffect(()=>{
        fetch(`http://ergast.com/api/f1/${searchParams.get('year')}/${searchParams.get('race')}/laps.json?limit=11000`)
        .then(data => data.json())
        .then(res => {
    ...
    return (
        ...
        <div className=" bg-slate-200 h-screen w-screen flex flex-col justify-center items-center">
            ...
            {
                drivers.map(driver => 
                <div className= 'flex space-x-6' key={`${race.season}${race.raceName}${driver}`} >
                    <DriverInfo className= 'flex-1 'id={driver} />
                    <RaceBar className='flex-2' color='bg-red-700' actualLap={actualLap} laps={70}></RaceBar>
                </div>
                )
            }
            </div>
